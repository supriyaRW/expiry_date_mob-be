...
function coerceIsoDate(text) {
  ...
  if (ddmmyyyy) {
    ...
    const p1 = parseInt(part1);
    const p2 = parseInt(part2);
    if (p1 > 12 && p2 <= 12) return `${year}-${String(part2).padStart(2, "0")}-${String(part1).padStart(2, "0")}`;
    if (p1 <= 12 && p2 > 12) return `${year}-${String(part1).padStart(2, "0")}-${String(part2).padStart(2, "0")}`;
    if (p1 <= 12 && p2 <= 12) return `${year}-${String(part2).padStart(2, "0")}-${String(part1).padStart(2, "0")}`;
  }
...
const prompt = `You are a precision OCR extractor for product labels. Extract exactly these fields as they appear.

### EXTRACTION TARGETS:
1. **mfgDate**: Manufacturing or Production date.
   - Keywords: "MFD", "Mfg Date", "Production Date", "PROD DATE", "Date of Fab", "Packed on", "PKD", "DOM".
   - CRITICAL: If no such date is found, return "". DO NOT guess or use today's date.
2. **expiryDate**: Expiration or Best Before date.
   - Keywords: "EXP", "Expiry", "Best Before", "Use By", "Valid Until", "BBE".
   - CRITICAL: If no such date is found, return "". DO NOT guess.
3. **batchNo**: Batch or Lot identifier.
   - Keywords: "Batch", "Lot", "BN", "B/N", "LOT#", "Batch#".
   - Extract exactly as written.

### STRICT RULES:
- If a field is missing, return "".
- NO hallucinations. If you aren't 99% sure, return "".
- mfgDate MUST be chronologically before expiryDate.
- Return ONLY single-line JSON.

### OUTPUT:
{"mfgDate": "YYYY-MM-DD", "expiryDate": "YYYY-MM-DD", "batchNo": "VALUE"}`;
...
app.post("/extract-fields", async (req, res) => {
  try {
    ...
    if (image) {
      const base64Data = image.includes(",") ? image.split(",")[1] : image;
      result = await model.generateContent([{ text: prompt }, { inlineData: { data: base64Data, mimeType: mimeType || "image/jpeg" } }]);
    } else {
      result = await model.generateContent([{ text: prompt }, { text: "TEXT:\n" + req.body.text }]);
    }
    ...
    try {
      const jsonStr = textResponse.match(/\{.*\}/s)?.[0] || textResponse;
      const parsed = JSON.parse(jsonStr);
      extracted.mfgDate = coerceIsoDate(parsed.mfgDate);
      extracted.expiryDate = coerceIsoDate(parsed.expiryDate);
      extracted.batchNo = String(parsed.batchNo || "").trim();
    } catch (e) {
      console.warn("Parse failed, falling back to regex");
    }
