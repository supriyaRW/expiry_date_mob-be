...
// const prompt = `You are an expert at extracting product information from product label images. Analyze the image and extract exactly THREE fields into a structured JSON format.

// ### FIELDS TO EXTRACT:

// 1. **product**: The primary name or title of the item visible in the image.
//    - Strategy: Look for the largest text, brand names, or labels like "Product:", "Item:", or "Name:". 
//    - Exclude: Batch numbers, weights (unless part of the name), or manufacturer addresses.
//    - Cleanliness: Remove leading/trailing punctuation or labels.
...
// 3. **batchNo**: The batch number, lot number, or serial number visible in the image.
...
//    - Search strategy: Scan the ENTIRE image text carefully. Look for ANY text containing these keywords.
...
const prompt = `You are an expert at extracting product information from product label images. Analyze the image and extract exactly THREE fields into a structured JSON format. Extract ONLY these fields - do not extract any other information.

### FIELDS TO EXTRACT:

1. **expiryDate**: The date the product expires or should be used by. THIS IS CRITICAL - MUST EXTRACT IF PRESENT.
   - Keywords (case-insensitive): "EXP", "Expiry", "Expiry Date", "Exp Date", "Best Before", "BBE", "BB", "Use By", "Use-by", "Use Before", "Valid Until", "Valid Thru", "Best By", "Consume By", "Expires", "Expires on", "Expiration", "Expiration Date", "Sell by", "Sell-by".
   - Search strategy: Scan the ENTIRE image text carefully. Look for ANY text containing these keywords (even if misspelled or abbreviated).
   - Date location: Date may appear on same line after keyword, next line, or separated by colon/dash/space.
   - Format: Convert to YYYY-MM-DD. Examples:
...
2. **batchNo**: The batch number, lot number, or serial number visible in the image.
   - Keywords (case-insensitive): "Batch", "Batch No", "Batch No:", "Batch Number", "Batch#", "Batch Code", "Batch ID", "LOT", "Lot", "LOT No", "Lot No", "LOT Number", "Lot Number", "LOT#", "Lote", "Lote No", "Serial", "Serial No", "Serial Number", "Serial#".
   - Search strategy: Scan the ENTIRE image text carefully. Look for ANY text containing these keywords.
   - Extraction: Extract the complete identifier that follows the keyword.
   - Include: All alphanumeric characters, hyphens (-), slashes (/), underscores (_).
...
3. **manufacturingDate**: The date the product was manufactured or produced. MUST EXTRACT IF PRESENT.
   - Keywords (case-insensitive): "Mfg", "MFG", "Mfg Date", "MFG Date", "Mfg:", "Manufacturing Date", "Manufactured", "Manufactured Date", "Production", "Production Date", "Prod", "Prod Date", "Produced", "Made", "Made on", "Made Date", "Date of Manufacture", "Produced on", "MFG:", "MFG Date:".
   - Search strategy: Scan the ENTIRE image text carefully. Look for ANY text containing these keywords (even if misspelled or abbreviated).
   - Date location: Date may appear on same line after keyword, next line, or separated by colon/dash/space.
   - Format: Convert to YYYY-MM-DD. Examples:
     * "12/05/2024" → "2024-05-12"
     * "15/03/2024" → "2024-03-15"
     * "03/15/2024" → "2024-03-15" (US format MM/DD/YYYY)
     * "15-03-2024" → "2024-03-15"
     * "15.03.2024" → "2024-03-15"
     * "Mar 15, 2024" → "2024-03-15"
     * "15 Mar 2024" → "2024-03-15"
     * "15/03/24" → "2024-03-15" (assume 20XX)
   - Logic: Manufacturing dates are PAST dates (not future dates like expiry dates).
   - If multiple manufacturing dates found, choose the earliest one.
   - If not found after thorough search, return "" (empty string).

### STRICT RULES:
- Extract ONLY these three fields: expiryDate, batchNo, manufacturingDate
- Do NOT extract any other information (no product names, no weights, no addresses, no other dates, no barcodes, etc.)
- Return ONLY a single-line JSON object.
- Do NOT include markdown formatting (no json blocks, no code fences).
- Do NOT include any conversational text or explanations.
- Use empty strings ("") for any field that cannot be identified with high confidence.
- Ensure all keys are lowercase as defined: expiryDate, batchNo, manufacturingDate
- ALL dates MUST be in YYYY-MM-DD format
- Read the ENTIRE image text carefully from top to bottom
- Expiry date is HIGHEST PRIORITY - extract it if present (these are FUTURE dates)
- Manufacturing date is IMPORTANT - extract it if present (these are PAST dates)
- Batch number is IMPORTANT - extract it if present

### OUTPUT FORMAT:
{"expiryDate": "YYYY-MM-DD", "batchNo": "String", "manufacturingDate": "YYYY-MM-DD"}`;
...
app.post("/extract-fields", async (req, res) => {
  try {
    ...
    let extracted = { expiryDate: "", batchNo: "", manufacturingDate: "" };
    ...
    try {
      ...
      // Extract manufacturingDate (also check for productionDate as fallback)
      const mfgDate = parsed.manufacturingDate || parsed.productionDate;
      if (mfgDate && String(mfgDate).trim()) {
        const rawMfg = String(mfgDate).trim();
        extracted.manufacturingDate = coerceIsoDate(rawMfg);
        console.log("Manufacturing date - raw:", rawMfg, "converted:", extracted.manufacturingDate);
      } else {
      ...
      // Try multiple patterns for manufacturingDate (also check productionDate)
      const manufacturingPatterns = [
        /"manufacturingDate"\s*:\s*"([^"]+)"/i,
        /manufacturingDate["\s]*:["\s]*"([^"]+)"/i,
        /manufacturingDate["\s]*:["\s]*([^",\s}]+)/i,
        /"productionDate"\s*:\s*"([^"]+)"/i,
        /productionDate["\s]*:["\s]*"([^"]+)"/i,
        /productionDate["\s]*:["\s]*([^",\s}]+)/i,
        /"mfgDate"\s*:\s*"([^"]+)"/i,
        /"production"\s*:\s*"([^"]+)"/i
      ];
